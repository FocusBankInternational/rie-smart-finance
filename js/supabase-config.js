...
// supabase-config.js — Configuración central
// ================================================================
...
async function obtenerUsuario() {
  ...
  return data || { id: user.id, nombres: user.email.split('@')[0], apellidos: '', moneda: 'USD', idioma: 'es' };
}
...
// Verificar sesión — redirige al login si no hay sesión activa
async function requireAuth() {
  ...
  if (el && usuario) el.textContent = (usuario.nombres || '') + ' ' + (usuario.apellidos || '');
  return usuario;
}
...
async function registrarUsuario(datos) {
  ...
  if (authData.user) {
    await db.from('usuarios').insert([{
      id: authData.user.id, nombres, apellidos, correo, pais, moneda, idioma: idioma || 'es', rol: 'usuario'
    }]);
  }
  return authData;
}
...
async function crearPrestamo(datos, cuotas) {
  ...
  const { data: prestamo, error } = await db.from('prestamos')
    .insert([{ ...datos, usuario_id: user.id }]).select().single();
  if (error) throw error;
  const cuotasConId = cuotas.map(c => ({ ...c, prestamo_id: prestamo.id }));
  await db.from('cuotas').insert(cuotasConId);
  return prestamo;
}
...
function amortizacionFrances(monto, tasaAnual, plazo, fechaInicio) {
  ...
  const tabla = []; let saldo = monto;
  let fecha = new Date(fechaInicio);
  for (let i = 1; i <= plazo; i++) {
    fecha = new Date(fecha); fecha.setMonth(fecha.getMonth() + 1);
...
function amortizacionAleman(monto, tasaAnual, plazo, fechaInicio) {
  ...
  for (let i = 1; i <= plazo; i++) {
    ...
    tabla.push({ numero_cuota: i, fecha_vencimiento: fecha.toISOString().split('T')[0], capital: capitalFijo, interes, cuota_total: round2(capitalFijo + interes), saldo_restante: saldo });
  }
  return tabla;
}
